...
import { SCRIPT_HEADER, VIDEO_SENTENCE, PIN_SENTENCE, buildProse, validateProse } from "./h3-prose";
import type { CallSheet, Shot } from "./types";

const mark = (characterId: string) => ({
  characterId,
  start: { x: 20, y: 50 },
  end: { x: 22, y: 50 },
  facing: 0,
...
function shot(dialogue: string, speaker?: string): Shot {
  return {
    id: "SH03",
    index: 2,
    heading: "3",
...
test("buildProse sample passes validateProse", () => {
  const text = wrap(buildProse(sheet(), shot("你仲記得個門口個燈？", "阿月")));
  validateProse(text);
  assert.ok(text.includes(VIDEO_SENTENCE));
  assert.ok(text.includes(PIN_SENTENCE));
});
...
test("dialogue present but unquoted throws", () => {
  const prose = buildProse(sheet(), shot("你仲記得個門口個燈？", "阿月")).replace(/"[^"]+"$/, "你仲記得個門口個燈？");
  assert.throws(() => validateProse(wrap(prose)), /spoken line missing/);
});

test("141 unquoted words blow the budget", () => {
  const pad = Array.from({ length: 141 }, (_, i) => `w${i}`).join(" ");

...
import { chatJsonWithFallback } from "./crew-llm";

/** Live door: chatJsonWithFallback(blender←nex) on SC-0915-LD0F SH01. */
async function main() {
  ...
  const schema = z.object({
    shotId: z.string(),
    camera: z.object({ lens_mm: z.number(), height_m: z.number(), aim: z.string().min(1) }),
    figures: z
      .array(
        z.object({
      ...
      .min(1),
    props: z.array(z.object({ name: z.string(), heldBy: z.string().nullable() })),
